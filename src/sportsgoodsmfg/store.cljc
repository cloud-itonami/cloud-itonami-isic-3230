(ns sportsgoodsmfg.store
  "SSoT for the sports-goods plant-operations coordination actor,
  ...
  3211`'s own `jewellerymfg.store`), this build ships a single
  `MemStore` backend only (atom of EDN) -- the deterministic default
  for dev/tests/demo, no deps. Per docs/adr/0001-architecture.md
  Decision 1, this vertical is self-contained (no external sports-
  goods-manufacturing capability library, no jurisdiction-scoped
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/impact-rating-percent/
                             ...
                             truth.
    - `equipment`         -- a molding/assembly/finishing unit's own
    ...
    - `maintenance`       -- a scheduled molding/assembly/finishing-
                             equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`sportsgoodsmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound sports-goods shipment
                             DRAFT (`sportsgoodsmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (sportsgoodsmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (sportsgoodsmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :ball
                :lot-number "SPG-2026-001" :impact-rating-percent 92
                :quantity-units 500.0 :weight-grams 210000.0
                :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :protective-gear
                :lot-number "SPG-2026-002" :impact-rating-percent 97
                :quantity-units 80.0 :weight-grams 36000.0
                :defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :racket
                :lot-number "SPG-2026-003" :impact-rating-percent 88
                :quantity-units 600.0 :weight-grams 180000.0
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `sportsgoodsmfg.registry` and
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `sportsgoodsmfg.registry` and
  returns {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
