(ns sportsgoodsmfg.governor
  "Sports Goods Plant Operations Governor -- the independent
  compliance layer that earns the SportsGoodsAdvisor the right to
  ...
  secretly tries to self-issue a product-safety/impact-protection
  ...
  stops being a coordination proposal and becomes direct molding/
  assembly-line-equipment control, so this MUST be a separate system
  able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:sports-goods-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    5. Product-safety-certification-
       authority blocked            -- ANY proposal (any op) whose own
                                       ...
                                       self-issue a product-safety/
                                       impact-protection compliance
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       let a fabricated classification
                                       through.
   11. Invalid impact-rating-
       percent                      -- for `:log-production-batch`, if
                                       the patch declares an
                                       `:impact-rating-percent` that is
                                       not a physically/certifiably
                                       plausible impact-protection
                                       compliance reading
                                       (`sportsgoodsmfg.registry/
                                       impact-rating-percent-valid?`),
   ...
   14. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [sportsgoodsmfg.registry :as registry]
...
(defn- safety-certification-authority-blocked-violations
  ...
  attempting to self-issue a product-safety/impact-protection
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-safety-certification? payload))
      [{:rule :safety-certification-authority-blocked
        :detail "self-issuing a product-safety/impact-protection compliance certification is permanently blocked -- exclusive authority of the accredited testing/certification body"}])))
...
(defn- invalid-impact-rating-violations
  "For `:log-production-batch`, if the patch declares an
  `:impact-rating-percent` that is not a physically/certifiably
  plausible impact-protection compliance reading, reject rather than
  let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rating (:impact-rating-percent (:value proposal))]
      (when (and (some? rating) (not (registry/impact-rating-percent-valid? rating)))
        [{:rule :invalid-impact-rating
          :detail (str rating " is outside the physically/certifiably plausible impact-rating-percent range")}]))))
...
(defn check
  "Censors a SportsGoodsAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-impact-rating-violations request proposal)
