(ns sportsgoodsmfg.operation
  "SportsGoodsOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (SportsGoodsAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Sports
  Goods Plant Operations Governor (:govern) and the rollout phase gate
  ...
  (:require [langgraph.graph :as g]
            ...
            [sportsgoodsmfg.advisor :as advisor]
...
(defn build
  "Compiles a SportsGoodsOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `sportsgoodsmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SportsGoodsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Sports Goods Plant Operations Governor -- independent censor.
      (g/add-node :govern
