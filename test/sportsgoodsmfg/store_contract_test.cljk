(ns sportsgoodsmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `sportsgoodsmfg.store` ns docstring for why a second (Datomic-
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :racket}})
    (is (= :racket (:product-type (store/batch s "batch-001"))))
