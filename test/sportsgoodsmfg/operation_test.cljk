(ns sportsgoodsmfg.operation-test
  "Smoke tests for the compiled SportsGoodsOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `sportsgoodsmfg.governor-contract-test`; the Store contract in
  `sportsgoodsmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "SportsGoodsOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "molding-001" :severity :moderate :description "impact-protection rating below threshold"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
