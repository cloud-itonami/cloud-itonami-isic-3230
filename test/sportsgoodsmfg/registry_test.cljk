(ns sportsgoodsmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [sportsgoodsmfg.registry :as r]))
...
(deftest known-product-types-are-valid
  (doseq [m [:ball :racket :protective-gear :fitness-equipment]]
    (is (r/product-type-valid? m))))

(deftest fabricated-product-type-is-invalid
  (is (not (r/product-type-valid? :unobtainium-gear)))
  (is (not (r/product-type-valid? nil))))

;; ----------------------------- impact-rating-percent-valid? -----------------------------

(deftest typical-impact-rating-percent-is-valid
  (is (r/impact-rating-percent-valid? 1))
  (is (r/impact-rating-percent-valid? 92))
  (is (r/impact-rating-percent-valid? 100)))

(deftest below-floor-impact-rating-percent-is-invalid
  (is (not (r/impact-rating-percent-valid? 0)))
  (is (not (r/impact-rating-percent-valid? -1))))

(deftest excessive-impact-rating-percent-is-invalid
  (is (not (r/impact-rating-percent-valid? 250)))
  (is (not (r/impact-rating-percent-valid? 101))))

(deftest non-integer-or-missing-impact-rating-percent-is-invalid
  (is (not (r/impact-rating-percent-valid? nil)))
  (is (not (r/impact-rating-percent-valid? 92.5)))
  (is (not (r/impact-rating-percent-valid? "92"))))

;; ----------------------------- weight-grams-valid? -----------------------------

(deftest typical-weight-grams-is-valid
  (is (r/weight-grams-valid? 0.5))
  (is (r/weight-grams-valid? 210000.0))
