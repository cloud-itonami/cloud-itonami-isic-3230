(ns sportsgoodsmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control molding/assembly/finishing
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue a product-safety/
  impact-protection compliance certification') implemented
  faithfully. The single invariant under test:

    SportsGoodsAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Sports Goods Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :ball}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :ball (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest safety-certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-safety-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the product-safety/impact-protection certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-product-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-type :unobtainium-gear}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-gear (:product-type (store/batch db "batch-001"))) "fabricated product-type never lands in the SSoT")))

(deftest invalid-impact-rating-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:impact-rating-percent 250}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-impact-rating} (-> (store/ledger db) last :basis)))
    (is (not= 250 (:impact-rating-percent (store/batch db "batch-001"))) "fabricated impact-rating-percent never lands in the SSoT")))
